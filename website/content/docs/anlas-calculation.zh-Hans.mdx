---
title: Anlas 计算
description: SDK 估算 Anlas 消耗的方式。
---
...
- `str(estimate)` 返回总 Anlas 值
- `int(estimate)` 也返回同一个总值
- low-level 的 `calculate_anlas(model, ImageParameters)` 无法判断未缓存 Vibe 的 encoding cost

详细的逆向分析请参考仓库 `docs/for-ai/anlas-calculation.md`。
