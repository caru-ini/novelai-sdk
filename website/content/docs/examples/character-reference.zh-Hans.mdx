---
title: 精准参考 (Precise Reference)
description: 用参考图像控制角色外观和艺术风格。
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="info">
此功能以前称为"角色参考 (Character Reference)"，于 2026 年 2 月更名为"精准参考"。
</Callout>

使用参考图像控制角色外观和艺术风格。
精准参考允许您在多次生成中保持一致的角色设计，并应用特定的艺术风格。
...
- **参考类型**:
  - `"character"`: 仅参考角色外观
  - `"style"`: 仅参考艺术风格
  - `"character&style"`: 同时参考 (默认)
- **精细控制**: 为每个参考调整 fidelity 和 strength
...
- **`image`** (必需): 参考图像 (文件路径、Base64 字符串或 PIL Image)
- **`type`**: 参考类型
- **`fidelity`**: 与参考的匹配度 (0.0–1.0, 默认: 1.0)
- **`strength`**: 多个参考时的相对权重 (0.0–1.0, 默认: 1.0)
